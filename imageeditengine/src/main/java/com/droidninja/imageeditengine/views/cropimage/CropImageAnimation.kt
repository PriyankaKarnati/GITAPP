 ...
 * zoom-in/out.
 */
class CropImageAnimation(// region: Fields and Consts
    ...
    private val mAnimPoints: FloatArray = FloatArray(8)
    private val mAnimMatrix: FloatArray = FloatArray(9)
    ...
    override fun applyTransformation(interpolatedTime: Float, t: Transformation) {
        ...
        for (i in mAnimPoints.indices) {
            mAnimPoints[i] = mStartBoundPoints.get(i) + (mEndBoundPoints.get(i) - mStartBoundPoints.get(i)) * interpolatedTime
        }
        mCropOverlayView.setBounds(mAnimPoints, mImageView.width, mImageView.height)
        for (i in mAnimMatrix.indices) {
            mAnimMatrix[i] = mStartImageMatrix.get(i) + (mEndImageMatrix.get(i) - mStartImageMatrix.get(i)) * interpolatedTime
        }
