...
import com.droidninja.imageeditengine.views.cropimage.CropOverlayView.CropWindowChangeListener
...
class CropImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet) : FrameLayout(context, attrs) {
    ...
    fun getAspectRatio(): Pair<Int?, Int?>? {
        return Pair(mCropOverlayView.getAspectRatioX(), mCropOverlayView.getAspectRatioY())
    }
     ...
     * bitmap requires saving it to file which can be expensive. default: false.
     */
    fun isSaveBitmapToInstanceState(): Boolean {
     ...
     * bitmap requires saving it to file which can be expensive. default: false.
     */
    fun setSaveBitmapToInstanceState(saveBitmapToInstanceState: Boolean) {
...
//    }
    ...
    fun setImageUriAsync(uri: Uri) {
        if (uri != null) {
            val currentTask = mBitmapLoadingWorkerTask?.get()
    ...
    fun startCropWorkerTask(
        ...
        if (bitmap != null) {
            mImageView.clearAnimation()
            val currentTask = mBitmapCroppingWorkerTask?.get()!!
            currentTask.cancel(true)
            reqWidth = if (options != RequestSizeOptions.NONE) reqWidth else 0
            reqHeight = if (options != RequestSizeOptions.NONE) reqHeight else 0
            val orgWidth = bitmap.width * mLoadedSampleSize
            val orgHeight = bitmap.height * mLoadedSampleSize
            mBitmapCroppingWorkerTask = if (mLoadedSampleSize > 1 || options == RequestSizeOptions.SAMPLING) {
                WeakReference(
 ...
 * Large Bitmaps Efficiently](http://developer.android.com/training/displaying-bitmaps/load-bitmap.html).
    ...
    interface OnSetCropOverlayReleasedListener {
         ...
         */
        open fun onCropOverlayReleased(rect: Rect?)
    }
    ...
    interface OnSetImageUriCompleteListener {
         ...
         */
        open fun onSetImageUriComplete(view: CropImageView?, uri: Uri?, error: Exception?)
    }
    // endregion
    ...
    interface OnCropImageCompleteListener {
         ...
         */
        open fun onCropImageComplete(view: CropImageView?, result: CropResult?)
    }
    // endregion
