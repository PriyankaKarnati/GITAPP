...
import com.droidninja.imageeditengine.views.cropimage.CropImageView.RequestSizeOptions
import java.io.*
...
object BitmapUtils {
    ...
    fun rotateBitmapByExif(bitmap: Bitmap, context: Context, uri: Uri): RotateBitmapResult? {
        ...
        try {
            ...
            if (iss != null) {
                ei = ExifInterface(iss)
                iss.close()
            }
        } catch (ignored: Exception) {
        }
        return ei?.let { rotateBitmapByExif(bitmap, it) } ?: RotateBitmapResult(bitmap, 0)
    }
    ...
    fun getRectTop(points: FloatArray): Float {
        return Math.min(Math.min(Math.min(points.get(1), points.get(3)), points.get(5)), points.get(7))
    }

    /** Get right value of the bounding rectangle of the given points.  */
    fun getRectRight(points: FloatArray): Float {
        return Math.max(Math.max(Math.max(points.get(0), points.get(2)), points.get(4)), points.get(6))
    }

    /** Get bottom value of the bounding rectangle of the given points.  */
    fun getRectBottom(points: FloatArray): Float {
        return Math.max(Math.max(Math.max(points.get(1), points.get(3)), points.get(5)), points.get(7))
    }
    ...
    private fun rotateAndFlipBitmapInt(
        ...
        return if (degrees > 0 || flipHorizontally || flipVertically) {
            ...
            matrix.postScale(if (flipHorizontally) {
                -1F
            } else {
                1.toFloat()
