...
class CropImageOptions : Parcelable {
    ...
    override fun writeToParcel(dest: Parcel, flags: Int) {
        ...
        dest.writeByte((if (showCropOverlay) 1 else 0) as Byte)
        dest.writeByte((if (showProgressBar) 1 else 0) as Byte)
        dest.writeByte((if (autoZoomEnabled) 1 else 0) as Byte)
        dest.writeByte((if (multiTouchEnabled) 1 else 0) as Byte)
        dest.writeInt(maxZoom)
        dest.writeFloat(initialCropWindowPaddingRatio)
        dest.writeByte((if (fixAspectRatio) 1 else 0) as Byte)
        ...
        dest.writeByte((if (allowRotation) 1 else 0) as Byte)
        dest.writeByte((if (allowFlipping) 1 else 0) as Byte)
        dest.writeByte((if (allowCounterRotation) 1 else 0) as Byte)
        dest.writeInt(rotationDegrees)
        dest.writeByte((if (flipHorizontally) 1 else 0) as Byte)
        dest.writeByte((if (flipVertically) 1 else 0) as Byte)
