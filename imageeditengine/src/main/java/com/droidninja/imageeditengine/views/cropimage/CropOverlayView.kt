...
import com.droidninja.imageeditengine.views.cropimage.CropImageView.CropShape
...
@JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private fun drawBackground(canvas: Canvas?) {
        ...
        if (mCropShape == CropShape.RECTANGLE) {
            if (!isNonStraightAngleRotated() || Build.VERSION.SDK_INT <= 17) {
                ...
                mPath!!.reset()
                mPath.moveTo(mBoundsPoints.get(0), mBoundsPoints.get(1))
                mPath.lineTo(mBoundsPoints.get(2), mBoundsPoints.get(3))
                mPath.lineTo(mBoundsPoints.get(4), mBoundsPoints.get(5))
                mPath.lineTo(mBoundsPoints.get(6), mBoundsPoints.get(7))
                mPath.close()
                canvas!!.save()
    ...
    private fun drawGuidelines(canvas: Canvas?) {
        if (mGuidelinePaint != null) {
            val sw: Float = if (mBorderPaint != null) mBorderPaint!!.getStrokeWidth() else 0F
            val rect = mCropWindowHandler!!.getRect()!!
            rect.inset(sw, sw)
    ...
    private fun drawBorders(canvas: Canvas?) {
        if (mBorderPaint != null) {
            val w = mBorderPaint!!.getStrokeWidth()
            val rect = mCropWindowHandler?.getRect()!!
            rect.inset(w / 2, w / 2)
            if (mCropShape == CropShape.RECTANGLE) {
    ...
    private fun drawCorners(canvas: Canvas?) {
        if (mBorderCornerPaint != null) {
            val lineWidth: Float = if (mBorderPaint != null) mBorderPaint!!.getStrokeWidth() else 0F
            val cornerWidth = mBorderCornerPaint!!.getStrokeWidth()
    ...
    private fun calculateBounds(rect: RectF?): Boolean {
        ...
        return if (!isNonStraightAngleRotated()) {
            ...
            var x0 = mBoundsPoints!!.get(0)
            var y0 = mBoundsPoints.get(1)
            var x2 = mBoundsPoints.get(4)
            var y2 = mBoundsPoints.get(5)
            var x3 = mBoundsPoints.get(6)
            var y3 = mBoundsPoints.get(7)
            if (mBoundsPoints.get(7) < mBoundsPoints.get(1)) {
                if (mBoundsPoints.get(1) < mBoundsPoints.get(3)) {
                    x0 = mBoundsPoints.get(6)
                    y0 = mBoundsPoints.get(7)
                    x2 = mBoundsPoints.get(2)
                    y2 = mBoundsPoints.get(3)
                    x3 = mBoundsPoints.get(4)
                    y3 = mBoundsPoints.get(5)
                } else {
                    x0 = mBoundsPoints.get(4)
                    y0 = mBoundsPoints.get(5)
                    x2 = mBoundsPoints.get(0)
                    y2 = mBoundsPoints.get(1)
                    x3 = mBoundsPoints.get(2)
                    y3 = mBoundsPoints.get(3)
                }
            } else if (mBoundsPoints.get(1) > mBoundsPoints.get(3)) {
                x0 = mBoundsPoints.get(2)
                y0 = mBoundsPoints.get(3)
                x2 = mBoundsPoints.get(6)
                y2 = mBoundsPoints.get(7)
                x3 = mBoundsPoints.get(0)
                y3 = mBoundsPoints.get(1)
            }
            val a0 = (y3 - y0) / (x3 - x0)
            val a1 = -1f / a0
            val b0 = y0 - a0 * x0
    ...
    interface CropWindowChangeListener {
         ...
         */
        open fun onCropWindowChanged(inProgress: Boolean)
    }
    // endregion
