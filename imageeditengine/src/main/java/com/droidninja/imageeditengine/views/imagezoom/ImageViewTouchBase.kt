...
abstract class ImageViewTouchBase : AppCompatImageView, IDisposable {
    ...
    protected open fun _setImageDrawable(drawable: Drawable?,
    ...
    protected fun fireOnLayoutChangeListener(left: Int, top: Int, right: Int,
                                             bottom: Int) {
    ...
    protected fun computeMaxZoom(): Float {
        val drawable: Drawable = drawable ?: return 1f
    ...
    override fun setImageMatrix(matrix: Matrix?) {
        val current: Matrix = imageMatrix
    ...
    protected fun getCenter(supportMatrix: Matrix?, horizontal: Boolean,
                            vertical: Boolean): RectF {
        val drawable: Drawable = drawable ?: return RectF(0F, 0F, 0F, 0F)
    ...
    protected fun updateRect(bitmapRect: RectF?, scrollRect: RectF) {
        ...
        if (bitmapRect.top + scrollRect.top >= 0
                && bitmapRect.bottom > mThisHeight) scrollRect.top = (0 - bitmapRect.top)
        if (bitmapRect.bottom + scrollRect.top <= mThisHeight - 0
                && bitmapRect.top < 0) scrollRect.top = (mThisHeight - 0 - bitmapRect.bottom)
        if (bitmapRect.left + scrollRect.left >= 0) scrollRect.left = (0 - bitmapRect.left)
        if (bitmapRect.right + scrollRect.left <= mThisWidth - 0) scrollRect.left = (mThisWidth - 0 - bitmapRect.right)
    }

    protected fun scrollBy(distanceX: Float, distanceY: Float,
                           durationMs: Double) {
        ...
        mHandler.post(object : Runnable {
            ...
            override fun run() {
                ...
                if (currentMs < durationMs) {
                    ...
                    if (centerRect.left != 0f || centerRect.top != 0f) scrollBy(centerRect.left, centerRect.top)
                }
            }
        })
    }
