...
//        mDrewDistance = if(isChecked){
//        val x = mLeftLineDistance + mRightLineDistance
        mDrewDistance = if (isChecked) {
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        mLeftLineDistance = Math.sqrt(Math.pow(mTickPoints[1]!!.x - mTickPoints[0]!!.x.toDouble(), 2.0) +
                Math.pow(mTickPoints[1]!!.y - mTickPoints[0]!!.y.toDouble(), 2.0)).toFloat()
        mRightLineDistance = Math.sqrt(Math.pow(mTickPoints[2]!!.x - mTickPoints[1]!!.x.toDouble(), 2.0) +
                Math.pow(mTickPoints[2]!!.y - mTickPoints[1]!!.y.toDouble(), 2.0)).toFloat()
    ...
    private fun drawTickPath(canvas: Canvas) {
        ...
        if (mDrewDistance < mLeftLineDistance) {
            val step: Float = if (mWidth / 20.0f < 3) {
                3f
            } else {
                (mWidth / 20.0f)
            }
